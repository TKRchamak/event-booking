...
export const updateUserByIdFromDB = async (userId: string, userData: IUser): Promise<IUser> => {
    try {
        // const user = await User.updateOne({ _id: userId }, { $set: userData }, { upsert: true });
        const user = await User.findByIdAndUpdate({ _id: userId }, { $set: userData }, { new: true });
        if (user?._id) {
            return user;
        } else {
